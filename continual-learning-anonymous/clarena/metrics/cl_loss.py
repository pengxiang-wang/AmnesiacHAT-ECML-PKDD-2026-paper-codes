...
class CLLoss(MetricCallback):
    ...
    - CSV files for classification loss (lower triangular) matrix and average classification loss. See `<ANONYMIZED_URL>` for details.
    - Coloured plot for test classification loss (lower triangular) matrix. See `<ANONYMIZED_URL>` for details.
    - Curve plots for test average classification loss over different training tasks. See `<ANONYMIZED_URL>` for details.

    Please refer to `<ANONYMIZED_URL>` to learn about this metric.
    """
    ...
    def __init__(
        ...
        **Args:**
        ...
        r"""Classification loss of training epoch. Accumulated and calculated from the training batches. See `<ANONYMIZED_URL>` for details. """
        self.loss_training_epoch: MeanMetricBatch
        r"""Total loss of training epoch. Accumulated and calculated from the training batches. See `<ANONYMIZED_URL>` for details. """

        # validation accumulated metrics
        self.loss_cls_val: MeanMetricBatch
        r"""Validation classification of the model loss after training epoch. Accumulated and calculated from the validation batches. See `<ANONYMIZED_URL>` for details. """

        # test accumulated metrics
        self.loss_cls_test: dict[int, MeanMetricBatch]
        r"""Test classification loss of the current model (`self.task_id`) on current and previous tasks. Accumulated and calculated from the test batches. Keys are task IDs and values are the corresponding metrics. It is the last row of the lower triangular matrix. See `<ANONYMIZED_URL>` for details. """
