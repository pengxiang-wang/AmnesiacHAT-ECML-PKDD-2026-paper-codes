...
class CLAccuracy(MetricCallback):
    ...
    - CSV files for test accuracy (lower triangular) matrix and average accuracy. See `<ANONYMIZED_URL>` for details.
    - Coloured plot for test accuracy (lower triangular) matrix. See `<ANONYMIZED_URL>` for details.
    - Curve plots for test average accuracy over different training tasks. See `<ANONYMIZED_URL>` for details.
    ...
    def __init__(
        ...
        **Args:**
        ...
        r"""Classification accuracy of training epoch. Accumulated and calculated from the training batches. See `<ANONYMIZED_URL>` for details. """

        # validation accumulated metrics
        self.acc_val: MeanMetricBatch
        r"""Validation classification accuracy of the model after training epoch. Accumulated and calculated from the validation batches. See `<ANONYMIZED_URL>` for details. """

        # test accumulated metrics
        self.acc_test: dict[int, MeanMetricBatch]
        r"""Test classification accuracy of the current model (`self.task_id`) on current and previous tasks. Accumulated and calculated from the test batches. Keys are task IDs and values are the corresponding metrics. It is the last row of the lower triangular matrix. See `<ANONYMIZED_URL>` for details. """
